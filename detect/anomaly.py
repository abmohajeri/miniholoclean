...
class AnomalyDetector(Detector):
    ...
    def detect_noisy_cells(self):
        ...
        for cluster_id in np.unique(model.labels_):
            ...
            for a in arr:
                search = self.df[self.df['HospitalName'] == a]
                cnt = h[h['HospitalName'] == a].iloc[0]['Count']
                if cnt < mean:
                    tmp_df = search['_tid_'].to_frame()
                    tmp_df.insert(1, 'attribute', 'HospitalName')
